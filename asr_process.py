...
matplotlib.use('TkAgg')  # 显式设置TkAgg后端

from plot.waveform import plot_evoked_joint

# 输入的原始 EEG 数据文件路径
input_file = './BCICIV_2a_labeled/A08T_raw.fif'
output_file = './BCICIV_2a_processed/A08T_asr_eeg.fif'
...
def main():
    ...
    epochs = mne.Epochs(raw, events, events_id, tmin=2, tmax=6, proj=True, baseline=None, preload=True,
    ...
    cleaned_avg = mne.Epochs(raw_cleaned, events, events_id, tmin=2, tmax=6, proj=True, baseline=None, preload=True,
                             picks=channel_names, event_repeated="drop")
    plot_evoked_joint(
        cleaned_avg,
        epochs,
